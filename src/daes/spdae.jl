...
struct SPDAE{dType <: Number, tType <: Real, arrayType <: AbstractArray{dType},
    ...
    function SPDAE(v::vType, f::fType, ϕ::ϕType, ψ::ψType,
            ...
            invariants::invType, parameters::parType,
            periodicity::perType) where {
        ...
        @assert all([ndims(q) == ndims(p) == ndims(λ) == ndims(μ)
                     for (q, p, λ, μ) in zip(q₀, p₀, λ₀, μ₀)])

        new{dType, tType, arrayType, vType, fType, ϕType, ψType, invType, parType, perType}(
            d, m, v, f, ϕ, ψ, t₀, q₀, p₀, λ₀, μ₀, invariants, parameters, periodicity)
    end
end

function _SPDAE(v, f, ϕ, ψ, t₀, q₀, p₀, λ₀, μ₀; invariants = NullInvariants(),
        parameters = NullParameters(), periodicity = NullPeriodicity())
    SPDAE(v, f, ϕ, ψ, t₀, q₀, p₀, λ₀, μ₀, invariants, parameters, periodicity)
end
...
function SPDAE(v, f, ϕ, ψ, t₀, q₀::StateVariable, p₀::StateVariable,
        λ₀::StateVariable, μ₀::StateVariable = zero(λ₀); kwargs...)
    SPDAE(v, f, ϕ, ψ, t₀, [q₀], [p₀], [λ₀], [μ₀]; kwargs...)
end
function SPDAE(v, f, ϕ, ψ, q₀::StateVariable, p₀::StateVariable,
        λ₀::StateVariable, μ₀::StateVariable = zero(λ₀); kwargs...)
    SPDAE(v, f, ϕ, ψ, 0.0, q₀, p₀, λ₀, μ₀; kwargs...)
end

function Base.hash(dae::SPDAE, h::UInt)
    hash(dae.d,
        hash(dae.m,
            hash(dae.v,
                hash(dae.f,
...
function Base.:(==)(dae1::SPDAE, dae2::SPDAE)
    (
        dae1.d == dae2.d
        && dae1.m == dae2.m
        && dae1.v == dae2.v
...
function Base.similar(
        equ::SPDAE, t₀::Real, q₀::StateVariable, p₀::StateVariable, λ₀::StateVariable,
        μ₀::StateVariable = initial_multiplier(q₀, equ.μ₀); parameters = equ.parameters)
...
function Base.similar(equ::SPDAE, q₀, p₀, λ₀ = initial_multiplier(q₀, equ.λ₀),
        μ₀ = initial_multiplier(q₀, equ.μ₀); kwargs...)
    similar(equ, equ.t₀, q₀, p₀, λ₀, μ₀; kwargs...)
end
function Base.similar(equ::SPDAE, t₀::Real, q₀::StateVariable, p₀::StateVariable,
        λ₀::StateVariable = initial_multiplier(q₀, equ.λ₀),
        μ₀::StateVariable = initial_multiplier(λ₀, equ.μ₀); kwargs...)
    similar(equ, equ.t₀, [q₀], [p₀], [λ₀], [μ₀]; kwargs...)
end
...
@inline GeometricBase.periodicity(equation::SPDAE) = hasperiodicity(equation) ?
                                                     equation.periodicity :
                                                     zero(equation.q₀[begin])
@inline initial_conditions(equation::SPDAE) = (
    equation.t₀, equation.q₀, equation.p₀, equation.λ₀, equation.μ₀)

function GeometricBase.functions(equation::SPDAE{DT, TT, AT, VT, FT, ϕT, ψT, IT,
        PT}) where {DT, TT, AT, VT, FT, ϕT, ψT, IT, PT <: NullParameters}
    NamedTuple{(:v, :f, :ϕ, :ψ)}((equation.v, equation.f, equation.ϕ, equation.ψ))
end

function GeometricBase.functions(equation::SPDAE{DT, TT, AT, VT, FT, ϕT, ψT, IT,
        PT}) where {DT, TT, AT, VT, FT, ϕT, ψT, IT, PT <: NamedTuple}

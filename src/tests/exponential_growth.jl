...
const ics = [(q = StateVariable(rand(1)),), (q = StateVariable(rand(1)),),
    (q = StateVariable(rand(1)),)]
...
function odeproblem(
        x₀ = x₀; parameters = default_parameters, tbegin = tbeg, tend = tend, Δt = Δt)
    ODEProblem(vectorfield, (tbegin, tend), Δt, x₀; parameters = parameters)
end

function odeensemble(
        ics = ics; parameters = default_parameters, tbegin = tbeg, tend = tend, Δt = Δt)
